...
exports.findAll = async () => {
    return await typeorm_1.getRepository(storeCategory_entity_1.StoreCategory).find({ relations: ['products'] }).catch(e => {
...
exports.buyProducts = async (data) => {
    // generate price
    const products = await typeorm_1.getRepository(product_entity_1.Product).findByIds(data.selectedProducts.map(item => {
        return item.id;
    })).catch(e => {
        logger_1.logger.error(e.message, e);
        throw new Error(error_constants_1.DatabaseErrorMessage);
    ...
    await typeorm_1.getRepository(cartItem_entity_1.CartItem).insert(cartItems).catch(e => {
        logger_1.logger.error(e.message, e);
        throw new Error(error_constants_1.DatabaseErrorMessage);
    });
...
exports.verify = async (reference) => {
    ...
    let refund;
    if (totalRefund !== 0) {
        refund = await payment_service_1.refundPayment(reference, totalRefund * 100, `Item(s) were already purchased by someone else while transaction with reference ${reference} was being processed`).catch(e => {
...
exports.createProduct = async (product) => {
    const existingProduct = await typeorm_1.getRepository(product_entity_1.Product).findOne({
        where: {
            name: product.name
        }
    }).catch(e => {
    ...
    await typeorm_1.getRepository(product_entity_1.Product).save(newProduct).catch(e => {
        logger_1.logger.error(`Product creation ${product.name}`, e.stack);
        throw e;
    });
    return true;
...
exports.createStoreCategory = async (data) => {
    ...
    typeorm_1.getRepository(storeCategory_entity_1.StoreCategory).save(category).catch(e => {
