...
export const findAll = async () => {
  return await getRepository(StoreCategory)
...
export const verify = async (reference: string) => {
  ...
  if (verified.status === "success") {
    for (let i = 0; i < transaction.cartItems.length; i++) {
      ...
      let productPurchasedString = `${element.count} unit(s) of ${
        product.name
      } at a total of ${(element.count * product.price).toLocaleString(
        "en-NG",
    ...
    sendMail(message)
      ...
      .catch((e) => {
        logger.error(
          `[Mail] sending for transaction ${transaction.reference} failed`,
          e.stack
        );
      });
  } else {
...
export const createProduct = async (product: CreateProductDto) => {
  const existingProduct = await getRepository(Product)
    .findOne({
      where: {
        name: product.name,
      },
  ...
  const category = await getRepository(StoreCategory).findOne(
