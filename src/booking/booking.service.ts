...
import {
  WORK_DAY_START_NORMAL,
  WORK_DAY_END_NORMAL,
  TIME_SLOT_INTERVAL,
  WORK_DAY_START_SUNDAY,
  WORK_DAY_END_SUNDAY,
...
export const createBooking = async (data: CreateBookingDto) => {
  ...
  const validity = await timeSlotsAreValid(
  ...
  if (validity.status) {
    result.created = true;
    booking = await getRepository(Booking)
      .save({
        name: data.name,
        email: data.email,
      ...
      .catch((e) => {
        ...
        logger.error(
          `Booking creation for 'email: ${data.email}' error - ${e.message}`,
          e.stack
        );
      });
  } else {
  ...
  if (booking && result.created) {
    ...
    const adminMessage: MailOptions = {
  ...
  logger.info(
    `Booking creation for 'email: ${data.email}' ending...`,
...
export const checkBooking = async (
  ...
  const booking = await getRepository(Booking)
    .findOne(
      {
        reference: reference,
      },
  ...
  if (booking.paidRequest) {
    ...
    if (!booking.payment.verified || !booking.payment.success) {
      ...
      const paymentVerified = await verifyPayment(booking.reference).catch(
        (e) => {
          throw e;
        }
      );
...
export const timeSlotsAreValid = async (
  ...
  return {
    status:
      ...
      !result.find(
        (item) =>
          item.payment && item.payment?.success && item.payment?.verified
      ),
    data: new Set(
      result.map((booking) => {
        if (
 ...
 */
export const getTimeSlotsByService = async (
  period: number,
  serviceName: string
): Promise<any> => {
  ...
  for (let i = 1; i < period; i++) {
    ...
    if (today.getDay() > 0) {
      ...
      for (
        let j = WORK_DAY_START_NORMAL;
        j < WORK_DAY_END_NORMAL;
        j = j + TIME_SLOT_INTERVAL
      ) {
      ...
      for (
        let j = WORK_DAY_START_SUNDAY;
        j < WORK_DAY_END_SUNDAY;
        j = j + TIME_SLOT_INTERVAL
      ) {
        today.setUTCHours(j, 0, 0, 0);
        if (today.getTime() > Date.now()) {
  ...
  const validity = await timeSlotsAreValid(allTimeSlots, serviceName).catch(
    (e) => {
      console.log(e);
      throw e;
    }
